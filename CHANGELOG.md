...
- Replace the historical Elicit attack framework with two prompt-only Find
  probes, three Ask references, exact ceilings, and compact corpus diagnostics.
- Consolidate release verification into `bash scripts/local_smoke.sh`, which
  regenerates splits and summaries, runs local gates, builds fresh artifacts,
  loads packaged tasksets, and records concise hashes without calling Prime.
- Remove obsolete fuzzy-contract tests, retired exploit fixtures, structural
