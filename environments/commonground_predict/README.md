...
Release 0.2.0 ships synthetic data only. Future human-session splits must use
reviewed, consented plaintext/public content, positional participant
pseudonyms, exact cluster partitions with `k >= 5`, and explicit source,
rights, schema-version, exporter, and privacy-review attestations. Automated
identifier screening does not replace human privacy and consent review.
The human-v2 intake shape is intentionally narrower than raw Context Engine
output: cluster `center` must be `[]`, and comment `extremity` and
`divisiveness` must be `null`, because those PCA-derived values are not
independently verified. Validation is capped at 10,000 participants, 1,000
statements, 1,000,000 vote cells, and 100 clusters.
...
against the shared fail-closed human governance contract before deterministic
masking.
...
([source](https://github.com/AgalmicSoftware/context-engine)); 0.2.0 contains no
real-human split.
